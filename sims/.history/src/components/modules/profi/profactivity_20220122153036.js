...
class profactivity extends React.Component {
        ...
        axios
            ...
            .then((response) => {
                if (response.status === 200) {
                    ...
                    this.updateTextField("GSTNo", data.GSTNo);
                    this.updateTextField("VATNo", data.VATNo);
                    ...
                    this.setState(
                        {
                            ...
                            ShippingID: parseInt(data.ShippingID),
    ...
    getBillingAddressByBillingID=(BillingID)=>{
        let Obj={};
        let BillingAddress=this.state.selectedCustomerObj.BillingAddress;
        for(let i=0;i<BillingAddress.length;i++){
          if(parseInt(BillingAddress[i].value)===parseInt(BillingID)){
            Obj=BillingAddress[i];
          }
        }
        return Obj;
    }
    getShippingAddressByShippingID=(ShippingID)=>{
        let Obj={};
        let ShippingAddress=this.state.selectedCustomerObj.ShippingAddress;
        for(let i=0;i<ShippingAddress.length;i++){
          if(parseInt(ShippingAddress[i].value)===parseInt(ShippingID)){
            Obj=ShippingAddress[i];
          }
        }
        return Obj;
    }
    ...
    updateFormValue = (key, e) => {
        ...
        switch (key) {
            ...
            case "GSTPlaceOfSold":
                ...
                let BillingAddressObj=this.getBillingAddressByBillingID(this.state.BillingID);
                let ShippingAddressObj=this.getShippingAddressByShippingID(this.state.ShippingID);
                if (this.state.selectedCustomerObj) {
                    if (e === false) {
                        try {
                            this.setState({
                                IsRegistedSupplier: BillingAddressObj.IsRegistedSupplier,
                                IsTaxExempt: BillingAddressObj.IsTaxExempt,
                                GSTNo: BillingAddressObj.GSTNo,
                                VATNo: BillingAddressObj.VATNo,
                                Reason: BillingAddressObj.Reason,
                                GSTPlaceOfSold: true,
                            });

                            this.updateTextField("GSTNo", BillingAddressObj.GSTNo);
                            this.updateTextField("VATNo", BillingAddressObj.VATNo);
                            this.updateTextField("Reason", BillingAddressObj.Reason);
                            // this.setState({
        ...
        try {
            ...
            switch (key) {
                ...
                case "LineDiscPercentage":
                    ProformaInvoiceLine[index].LineDiscPercentage = parseFloat(value);
                    this.setState({ ProformaInvoiceLine: ProformaInvoiceLine });
                    break;
                    break;
                default:
        ...
        const breadcrumbHtml = (
            <Fragment>
                <Breadcrumb
                    ...
                    masterLinkTitle="Proforma Invoice"
                                ...
                                <Grid container spacing={0} >
                                    <Grid item xs={12} sm={12} md={12} lg={12}>
                                            ...
                                            <AccordionDetails
                                                ...
                                                <div style={{ height: 250, width: '100%', overflowY: 'scroll', overflowX: 'scroll' }}>
                                                    <POL
                                                        ...
                                                        triggeronKeyDown={this.triggeronKeyDown}
                                                    />
                                                </div>
                                            </AccordionDetails>
                                        </Accordion>
                                        ...
                                        <Accordion
                                            style={disabledStyle}
                                            key="a-8"
                                            expanded={this.state.accordion8}
                                            className="accordionD"
                                        >
                                            <AccordionSummary
                                                className="accordion-Header-Design"
                                                expandIcon={<ExpandMoreIcon onClick={(e) => handleAccordionClick("accordion8", e)} />}
                                                aria-controls="panel1a-content"
                                                id="accordion8"
                                                style={{ minHeight: "40px", maxHeight: "40px" }}
                                                onClick={(e) => handleAccordionClick("accordion8", e)}
                                            >
                                                <Typography
                                                    key="GD-Activity"
                                                    className="accordion-Header-Title"
                                            ...
                                            <AccordionDetails
                                                key="accordion8" className="AccordionDetails-css">
                                                <Fragment>
                                                    <Grid container spacing={0}>
                                                        <Grid item xs={12} sm={12} md={12} lg={12}>
                                                            &nbsp;
